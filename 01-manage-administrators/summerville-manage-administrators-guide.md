_Summerville Admin Console  ›  Manage Administrators_

# Manage Administrators

> Who has console access, and what they've done with it.

## Step-by-Step Workflow

### Step 1 — Manage Admin Users

Opens the roster. Default lookup is User ID radio + search.

![Step 1 — Manage Admin Users](img/ma-01-admin-users-landing.jpg)

### Step 2 — Search by Name

Toggle the radio to Name. First and Last name fields replace User ID — faster when you only have HR's employee list.

![Step 2 — Search by Name](img/ma-02-admin-users-by-name.jpg)

### Step 3 — Add New Admin User

Capture Title, Description, Email, User ID, Mobile, and Role. Role is the access boundary — map it to the signed-off access matrix.

![Step 3 — Add New Admin User](img/ma-03-add-new-admin-user.jpg)

### Step 4 — Admin Activity

The audit log. Start Date and End Date are mandatory so every extract is scoped and reproducible.

![Step 4 — Admin Activity](img/ma-04-admin-activity.jpg)

### Step 5 — Date fields required

Clicking Search without dates returns "Please select start date / end date" — prevents unscoped dumps.

![Step 5 — Date fields required](img/ma-05-admin-activity-validation.jpg)

## Summary

Two pages: Manage Admin Users for who has access, Admin Activity for what they did. Separation is deliberate — provisioning and audit get reviewed on different cadences.

## Key Use Cases

- New Treasury hire → Add New Admin User, Role = Treasury-Ops.
- Quarterly recertification → search by Name, reconcile with HR's list.
- Examiner asks for a log → Admin Activity with the exam window.
- SOC flags a change → Admin Activity to attribute it to an admin + timestamp.
