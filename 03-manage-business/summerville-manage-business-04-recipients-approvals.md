...
Click a row for name, bank, routing/account masking, recipient type, and verification status. Confirm against supporting documents, then mark Verified.
...
Recipients = who the business pays. Approval Settings = how many people sign off before it goes out. Together they are the first line against BEC and check-kiting.

## Key Use Cases

- Phishing attempt reported → trim stale recipients, raise high-value ACH and wire approvals to 2.
- Quarterly KYC → verify each recipient against supporting docs, keep the low-approval path defensible.
